...
Dynamicweb.PIM.BulkEdit.prototype._enableBulkEditRibbonButton = function () {
    ...
    BulkEdit.on("click", function () {
        dwGrid_FieldsGrid.deleteRows(dwGrid_FieldsGrid.rows.getAll());
        self.selectMultiEditLanguage();
        dialog.show('MultiEditDlg');
    });
};
...
Dynamicweb.PIM.BulkEdit.prototype.updateFields = function (selectedProductIds, fieldUpdateFn) {
    ...
    fieldUpdateFn = fieldUpdateFn || function (fieldDefinition, editedFieldObj) {
        if (fieldDefinition && !self.isFieldEditorReadonly(fieldDefinition)) {
            self._setFieldValue(fieldDefinition, editedFieldObj.value);
        }
    };
...
Dynamicweb.PIM.BulkEdit.prototype._setFieldValue = function (fieldDifinition, updatedValue) {
    switch (fieldDifinition.controlType) {
        case "Filemanager":
            this._setFileManageControlValue(fieldDifinition.controlId, updatedValue);
            break;
        case "Date":
        case "DateTime":
            var datePickerId = fieldDifinition.controlId.substr(0, fieldDifinition.controlId.length - "_label".length);
            if (!updatedValue || updatedValue.substr(0, 4) === "2999") {
                DWClearDate(datePickerId);
            } else {
                Dynamicweb.UIControls.DatePicker.get_current().dict[datePickerId].setDate(updatedValue);
            }
            break;
        case "Bool":
        case "Checkbox":
            document.getElementById(fieldDifinition.controlId).checked = updatedValue;
            break;
        case "EditorText":
            document.getElementById(fieldDifinition.controlId.substring(0, fieldDifinition.controlId.length - "_editor".length)).value = updatedValue;
            document.getElementById(fieldDifinition.controlId).innerHTML = updatedValue;
            break;
        case "Select":
            var fields = document.getElementsByName(fieldDifinition.controlId);
            fields[0].value = updatedValue;
            break;
        case "MultiSelectList":
            var field = document.getElementsByName(fieldDifinition.controlId)[0];
            for (var i = 0; i < field.options.length; i++) {
                var option = field.options[i];
                option.selected = updatedValue.length && updatedValue.indexOf(option.value) > -1;
            }
            break;
        case "RadioButtonList":
        case "CheckBoxList":
            ...
            for (var i = 0; i < fields.length; i++) {
                var field = fields[i];
                field.checked = updatedValue.length && updatedValue.indexOf(field.value) > -1;
            }
            break;
        case "GroupDropDownList":
            var selEl = document.getElementById(fieldDifinition.controlId);
            this._populateDropDownList(selEl, fieldDifinition.predefinedValues, updatedValue);
            break;
        default:
            document.getElementById(fieldDifinition.controlId).value = updatedValue;
            break;
    }
};
...
Dynamicweb.PIM.BulkEdit.prototype._getControlDefinition = function (controlId) {
    return this._controlDefinitions.find(function (definition, index) {
        return definition.controlType == "EditorText" ? definition.controlId.lastIndexOf(controlId + "_editor") > -1 : definition.controlId.lastIndexOf(controlId) > -1;
    });
};
...
Dynamicweb.PIM.BulkEdit.prototype.productFieldsBulkEdit = function (productId) {
    var url = "/Admin/Module/eCom_Catalog/dw7/PIM/FieldsBulkEdit.aspx?ProdId={productId}&groupId={groupId}&queryId={queryId}";
