...
-->
  ...
  <xsl:template match="//Page">
    ...
    <div>
      <xsl:if test="@AbsoluteLevel = 2">
        <xsl:attribute name="class">expand-container</xsl:attribute>
        <label>
          <xsl:attribute name="class">expand-container__btn expand-container__btn--absolute menu-left__item--top-level dw-mod</xsl:attribute>
          <xsl:attribute name="for">LeftMenuExpandableCheck_<xsl:value-of select="@ID"></xsl:value-of>_<xsl:value-of select="position()"></xsl:value-of>
            ...
            <a>
              <xsl:attribute name="class">
                menu-left__link menu-left__link--top-level 
      ...
      <xsl:if test="@AbsoluteLevel = 3">
        <xsl:attribute name="class">
          <xsl:if test="@Active='True'"> active </xsl:if> menu-left__level-<xsl:value-of select="@AbsoluteLevel"></xsl:value-of>
        </xsl:attribute>
        <a>
          <xsl:attribute name="class">
            menu-left__link menu-left__link--level-<xsl:value-of select="@AbsoluteLevel"></xsl:value-of>
            <xsl:if test="@Active='True'"> active </xsl:if>
            dw-mod
          </xsl:attribute>
          <xsl:attribute name="href">
      ...
      <xsl:if test="count(Page)">
        <xsl:if test="@AbsoluteLevel = 2">
          <div class="expand-container__content menu-left dw-mod">
